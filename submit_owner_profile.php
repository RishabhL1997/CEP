...
// $sql = "INSERT INTO owner_profile (user_email, user_type, permanent_address, alt_contact_number, aadhaar_card_path, upi_qr_path, upi_id, bank_account_number, ifsc_number) 
//         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
$stmt->bind_param("sssssssss",$user_email, $user_type, $permanent_address, $alt_contact_number, $aadhaar_card_path, $upi_qr_path, $upi_id, $bank_account_number, $ifsc_number);

if ($stmt->execute()) {
    $stmt->close();
    $conn->close();
    echo "Owner profile submitted successfully.";
